...
void find_pts_in_plane(pcl::PointCloud<pcl::PointXYZ> cloud, std::vector<cv::Point3d> rand_pts){ //cv::Mat

	std::cout << "Inside function" << std::endl;
	//std::cout << rand_pts[1].x << rand_pts[1].y << rand_pts[1].z << std::endl;
	std::cout << rand_pts[1].x << rand_pts[1].y << rand_pts[1].z << std::endl;
	//std::cout << rand_pts[3].x << rand_pts[3].y << rand_pts[3].z << std::endl;

	cv::Point3d plane_eq=plane_calc_determinant(rand_pts);

	if (cloud != NULL && cloud.points.size() == 307200) {
		for (int y = 0; y < 480; y++) {
			for (int x = 0; x < 640; x++) {
				int pixnum = px(x, y);
...
//  	  cout << "Times passed in seconds: " << t << endl;
	  ...
	  while (run_me){
		  ...
		  int pixel_x = rand() % 560 + 1;
		  pixel_x+=40;
		  int pixel_y = rand() % 400 + 1;
		  pixel_y+=40;

		  int pt_1_index=px(pixel_x+20,pixel_y);
		  int pt_2_index=px(pixel_x,pixel_y+20);
		  int pt_3_index=px(pixel_x,pixel_y-20);

//		  std::cout << cloud[pt_1_index].x << cloud[pt_1_index].y << cloud[pt_1_index].z << std::endl;
		  Point3d point_1;
		  point_1.x=cloud[pt_1_index].x;
		  point_1.y=cloud[pt_1_index].y;
		  point_1.z=cloud[pt_1_index].z;
		  Point3d point_2;
		  point_2.x=cloud[pt_2_index].x;
		  point_2.y=cloud[pt_2_index].y;
		  point_2.z=cloud[pt_2_index].z;
		  Point3d point_3;
		  point_3.x=cloud[pt_3_index].x;
		  point_3.y=cloud[pt_3_index].y;
		  point_3.z=cloud[pt_3_index].z;

		  std::cout << "Inside callback" << std::endl;
		  std::cout << point_2.x << point_2.y << point_2.z << std::endl;

		  std::vector<cv::Point3d> rand_pts;
		  rand_pts.push_back(point_1);
		  rand_pts.push_back(point_2);
		  rand_pts.push_back(point_3);

		  cv::Point3d plane_eq;

		  plane_eq = plane_calc_determinant(rand_pts);

		  find_pts_in_plane(cloud,rand_pts);
